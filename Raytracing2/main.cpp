...
Vector renduCouleur(const Ray &r, const Scene& s, int nombreRebond){
        ...
        if(has_inter){
                ...
                Vector color_of_sphere= s.spheres[sphere_indice].colorS;
...
int main()
{
    ...
    Sphere sphere1(Vector(0,0,-55),5, Vector(0,1,0),false, true);
    Sphere sphere2(Vector(0,22,-55),5, Vector(0,0,1),true,false);
    Sphere sphere3(Vector(0,12,-55),5, Vector(1,0,1));
    Sphere sphere4(Vector(0,-15,-55),5, Vector(1,1,0),true,false);
    ...
    for(int i = 0 ; i<H ; i++){
        ...
        Vector couleur= renduCouleur(r, scene,5);
...
save_image("image13.png", &img[0], W, H);
    return 0;
}
